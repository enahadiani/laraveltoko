...
<!-- LIST DATA -->
<x-list-data judul="Konsinyasi Pembayaran" tambah="true"
    :thead="array('No Bukti','Tanggal','Harga','Jumlah','Stok','Total','')"
    :thwidth="array(15,15,15,15,10,10,0)" :thclass="array('','','','','','','text-center')" />
<!-- END LIST DATA -->
<style>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    <label for="nilai_bayar">Nilai Beli</label>
                                    <input class="form-control currency" type="text" placeholder="nilai_bayar Pemberi"
                                        readonly id="nilai_bayar" name="nilai_bayar" value="0">
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="col-md-8 col-sm-12">
                                ...
                                <div class="input-group">
                                    <div class="input-group-prepend hidden" style="border: 1px solid #d7d7d7;">
                                        <span class="input-group-text info-code_kode_vendor" readonly="readonly"
                                            title="" data-toggle="tooltip" data-placement="top"></span>
                                    </div>
                                    <input type="text" class="form-control inp-label-kode_vendor" id="kode_vendor"
                                        name="kode_vendor" value="" title="" readonly>
                        ...
                        <div class="form-group col-md-6 col-sm-12">
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    ...
                                    <input class="form-control currency" type="text" placeholder="Total Pembelian"
                                        readonly id="total_bayar" name="total_bayar" value="0">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                        <div class="form-group col-md-6 col-sm-12">
                                        <div class="row">
                                            <div class="col-md-6 col-sm-12">
                                                ...
                                                <div class="input-group">
                                                    <div class="input-group-prepend hidden" style="border: 1px solid #d7d7d7;">
                                                        <span class="input-group-text info-code_no_hutang" readonly="readonly" title="" data-toggle="tooltip" data-placement="top" ></span>
                                                    </div>
                                                    <input type="text" class="form-control inp-label-no_hutang" id="no_hutang" name="no_hutang" autocomplete="off" data-input="cbbl" value="" title=""  readonly>
                                                    <span class="info-name_no_hutang hidden">
                                                        <span></span> 
                                                    </span>
                                                    <i class="simple-icon-close float-right info-icon-hapus hidden"></i>
                                                    <i class="simple-icon-magnifier search-item2" id="search_no_hutang"></i>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                        <div class="form-group col-md-6 col-sm-12">
                            <div class="row">
                                ...
                                <div class="col-md-6 col-sm-12">
                                    <label for="nilai_retur">Nilai Retur</label>
                                    <input class="form-control currency" type="text" placeholder="nilai_retur Pemberi"
                                        readonly id="nilai_retur" name="nilai_retur" value="0">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    function generateNoBukti(){
        ...
        var url = "{{url('esaku-trans/konsinyasi-nobukti')}}";
        $.ajax({
            type: 'GET',
            url : url,
            data: {
    ...
    var dataTable = generateTable(
        "table-data",
        "{{ url('esaku-trans/konsinyasi-index') }}",
        [{
                "targets": 0,
    ...
    function hitungTotal(){
        ...
        $('#input-bill tbody tr').each(function(index) {
            var harga = toNilai($(this).find('.inp-harga').val())
            var selisih = toNilai($(this).find(`.tdmodulke${index + 1}`).text())
            total2= harga * selisih;
            
            $(this).find(`.tdtotalke${index + 1}`).text(total2);
        });
    }
    ...
    function hitungNilairetur(){
        ...
        $('#input-bill tbody tr').each(function(index) {
            ...
            nilairetur= sop * harga;
        });

        $('#nilai_retur').val(nilairetur);
    }
    ...
    //Hitung Nilai Bayar
    function hitungNilaibayar(){
        var nilaibayar  = 0;
        var total = formatNumber($('#total_bayar').val())
        var retur = formatNumber($('#nilai_retur').val())
        console.info(retur)
            console.info(total)
            nilaibayar= total + retur;
            console.log(nilaibayar)

        $('#nilai_bayar').val(nilaibayar);
    }
    ...
    function hitungSelisih(){
        var nilaisel = 0;
        $('#input-bill tbody tr').each(function(index) {
            var jumlah = toNilai($(this).find('.inp-jumlah').val())
            var sop = toNilai($(this).find('.inp-sop').val())
            nilaisel= jumlah - sop;
            
            $(this).find(`.tdmodulke${index + 1}`).text(nilaisel);
        });
    }
    //END Hitung selisih
    ...
    $('#saku-datatable').on('click', '#btn-tambah', function() {
        ...
        $('#row-id').hide();
        $('#method').val('post');
        $('#judul-form').html('Tambah Data Konsinyasi Pebelian');
    ...
    $('#form-tambah').validate({
        ...
        submitHandler: function (form) {
            ...
            var url = "{{ url('esaku-trans/konsinyasi-bayar') }}";
    ...
    function loadData(no_hutang){
        var url = "{{url('esaku-trans/get-loaddata')}}";
        $.ajax({
            type: 'GET',
            url: url,
            data: {
            ...
            success: function(result){
                ...
                if(data.length > 0){
                    ...
                    for (let i = 0; i < data.length; i++) {
                            ...
                            html += "</div></td>";
                            html += "<td><div>";
                            html += "<span class='td-bank_trans tdbank_transke"+no+"'>"+line[i].nama_barang+"</span>";
                            html += "<input type='text' name='nama_barang[]' class='inp-bank_trans form-control bank_transke"+no+" hidden'  value='"+line[i].nama_barang+"'  >";
                            html += "</div></td>";
                            html += "<td><div>";
                            html += "<span class='td-satuan tdsatuanke"+no+"'>"+line[i].satuan+"</span>";
                            html += "<input type='text' name='satuan[]' class='inp-satuan form-control satuanke"+no+" hidden'  value='"+line[i].satuan+"'  >";
                            html += "</div></td>";
                            html += "<td class='text-right'>"
                            html += "<span class='td-nilai tdnilaike"+no+"'>"+format_number(line[i].harga)+"</span>"
                            html += "<input type='text' name='harga[]' class='inp-harga form-control nilaike"+no+" hidden currency'  value='"+parseFloat(line[i].harga)+"' >"
                            html += "</td>"
                            html += "<td><div>";
                            html += "<span class='td-keterangan tdketeranganke"+no+"'>"+format_number(line[i].jumlah)+"</span>";
                            html += "<input type='text' name='jumlah[]' class='inp-jumlah form-control keteranganke"+no+" hidden currency'  value='"+parseFloat(line[i].jumlah)+"'  >";
                            html += "</div></td>";
                            html += "<td>"
                            html += "<span class='td-status tdstatuske"+no+" tooltip-span'></span>";
                            html += "<input type='text' name='stok[]' id='inp-sop' class='inp-sop form-control nilaike"+no+" currency '  value='"+parseFloat(line[i].stok)+"' >"
                            html += "</td>"
                            html += "<td><div>";
                            html += "<span class='td-modul tdmodulke"+no+"'>"+format_number(line[i].selisih)+"</span>";
                            html += "<input type='text'  name='modul[]' class='inp-selisih form-control modulke"+no+" hidden currency'  value='"+parseFloat(line[i].selisih)+"'  >";
                            html += "</div></td>";
                            html += "<td class='text-right'>"
                            html += "<span class='td-nilai tdtotalke"+no+"'>"+format_number(line[i].total)+"</span>"
                            html += "<input type='text' name='total[]' class='inp-total form-control nilaike"+no+" hidden currency'  value='"+parseFloat(line[i].total)+"' >"
                            html += "</td>"
                            no++;
                    }
