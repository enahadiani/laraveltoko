    ...
    <!-- LIST DATA -->
    <x-list-data judul="Data Stok" tambah="true" :thead="array('No Bukti','Tanggal','Keterangan','Kode Gudang','Tgl Input','Aksi')" :thwidth="array(15,10,55,10,0,10)" :thclass="array('','','','','','text-center')" />
    <!-- END LIST DATA -->
    <style>
                            ...
                            <div class="form-group col-md-6 col-sm-12">
                                <div class="row">
                                    ...
                                    <div class="col-md-6 col-sm-12">
                                        ...
                                        <input class='form-control currency' type="text" id="total" name="total" required readonly>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-content tabcontent-border col-12 p-0">
                            <div class="tab-pane active" id="data-barang" role="tabpanel">
                                ...
                                <div class='col-md-12 table-responsive' style='margin:0px; padding:0px;'>
                                    ...
                                    <button type="button" id="tambah-input-barang" class="btn btn-block add-row add-row-input-barang add-row">
    ...
    <script>
        ...
        function generateNoBukti(tanggal){
            ...
            $.ajax({
                type: 'GET',
                url: "{{ url('esaku-trans/stok-nobukti') }}",
                dataType: 'json',
                async:false,
        ...
        function editData(id){
            ...
            $('#judul-form').html('Edit Data Stok');
            $('#form-tambah')[0].reset();
            $('#form-tambah').validate().resetForm();
            $("#kode_gudang").parents('.input-group').addClass('readonly');
            $.ajax({
                type: 'GET',
                url: "{{ url('/esaku-trans/stok') }}/"+id,
                dataType: 'json',
                async:false,
            ...
            $.ajax({
                ...
                success:function(result){
                    if(result.data.status){
                        ...
                        showNotification("top", "center", "success",'Hapus Data','Data Stok ('+id+') berhasil dihapus ');
        ...
        $('#saku-datatable').on('click', '#btn-tambah', function(){
            $('#row-id').hide();
            $('#method').val('post');
            $('#judul-form').html('Tambah Data Stok');
        ...
        $('#form-tambah').validate({
            ...
            submitHandler: function (form) {
                ...
                var arr_numeric = ['jumlah']; //format kolom nilai 
                var exclude_keys = ['nama_barang']; // kolom yang ingin dihapus
                var details = stokTable.getData().map(row => {
                ...
                // $iconLoad.show();
                if(param == "edit"){
                    var url = "{{ url('/esaku-trans/stok') }}/"+id;
                }else{
                    var url = "{{ url('/esaku-trans/stok') }}";
                }
        ...
        // ENTER FIELD FORM
        $('#tanggal,#no_bukti,#keterangan,#kode_gudang,#total').keydown(function(e){
            var code = (e.keyCode ? e.keyCode : e.which);
            var nxt = ['tanggal','no_bukti','keterangan','kode_gudang','total'];
            if (code == 13 || code == 40) {
                e.preventDefault();
        ...
        // GRID 
        function getBarang(idx, kode_gudang, id){
            var tmp = id.split(" - ");
            kode = tmp[0];
            $.ajax({
                type: 'GET',
                url: "{{ url('esaku-trans/stok-barang') }}",
                dataType: 'json',
                data:{
                ...
                success:function(result){    
                    if(result.status){
                        if(typeof result.data !== 'undefined' && result.data.length>0){
                            ...
                            stokTable.setCellSilent(idx, 'kode_barang', line.kode_barang);
                            stokTable.renderCell(idx, 'kode_barang');
                            stokTable.setCellSilent(idx, 'nama_barang', line.nama);
                            stokTable.renderCell(idx, 'nama_barang');
                            stokTable.setCellSilent(idx, 'satuan', line.satuan);
                            stokTable.renderCell(idx, 'satuan');
                            stokTable.setCellSilent(idx, 'jumlah', 0);
                            stokTable.renderCell(idx, 'jumlah');
                            stokTable.focusCell(idx, "jumlah");
                        }else{
                            stokTable.setCellSilent(idx, 'kode_barang', '');
                            stokTable.renderCell(idx, 'kode_barang');
                            stokTable.setCellSilent(idx, 'nama_barang', '');
                            stokTable.renderCell(idx, 'nama_barang');
                            stokTable.setCellSilent(idx, 'satuan', '');
                            stokTable.renderCell(idx, 'satuan');
                            stokTable.setCellSilent(idx, 'jumlah', 0);
                            stokTable.renderCell(idx, 'jumlah');
                            stokTable.focusCell(idx, "kode_barang");
                            msgDialog({
                                id: '-',
                                type: 'warning',
        ...
        var stokTable = new EditableTablePlugin("#input-barang", {
            ...
            inputs: {
                    ...
                    beforeSearch: function(){
                        if($('#kode_gudang').val() == ""){
                            msgDialog({
                                ...
                                text: 'Gudang harus dipilih terlebih dahulu!',
                            });
                            return false;
                        }else return true;
                    },
                    searchOptions: function(index) {
                        return {
                            url: "{{ url('esaku-trans/stok-barang') }}",
                            judul: "Daftar Barang",
                            header: ['Kode', 'Nama'],
                            ...
                            onItemSelected: function(data) {
                                stokTable.setCellSilent(index, 'kode_barang', data.kode_barang);
                                stokTable.setCellSilent(index, 'nama_barang', data.nama);
                                stokTable.setCellSilent(index, 'satuan', data.satuan);
                                stokTable.setCellSilent(index, 'jumlah', 0);
                                stokTable.renderCell(index, 'kode_barang');
                                stokTable.renderCell(index, 'nama_barang');
                                stokTable.renderCell(index, 'satuan');
                                stokTable.renderCell(index, 'jumlah');
                                stokTable.focusCell(index, "jumlah");
                            }
                        } 
                    }
                },
                ...
                jumlah: {type: "currency", required: true}
            },
            data: [],
            onChange: function(index, key, value) {
                if(key == "kode_barang"){
                    getBarang(index, $('#kode_gudang').val(), value);
                }
                if(key == "jumlah"){
                    hitungTotal();
                }
            },
            onDeleteRow: function(index, rowData){
                hitungTotal();
            }
        });

        $('.add-row-input-barang').on('click',function(){
            stokTable.addRow();
        });
    </script>