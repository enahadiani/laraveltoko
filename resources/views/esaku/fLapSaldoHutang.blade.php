...
<div class="row" id="saku-filter">
    <div class="col-12">
        <div class="card" >
            <x-report-header judul="Laporan Hutang"/>
                            ...
                            <div class="px-4 pb-4 pt-2">
                                <form id="form-filter">
                                    ...
                                    <div id="inputFilter">
                                        ...
                                        <x-inp-filter kode="periode" nama="Periode" selected="4" :option="array('3','4')"/>
                                        <x-inp-filter kode="kode_gudang" nama="Gudang" selected="3" :option="array('3')"/>
                                        <x-inp-filter kode="kode_vendor" nama="Vendor" selected="1" :option="array('1','2','3','i')"/>
                                        <x-inp-filter kode="jenis" nama="Jenis" selected="3" :option="array('1','3')"/>
                                        <!-- END COMPONENT -->
                                    </div>
...
<x-report-result judul="Laporan Hutang" padding="px-4 py-4"/>
...
<script type="text/javascript">
            ...
            success:function(result){   
                if(result.status){
                    ...
                    generateRptFilter('#inputFilter',{
                        kode : ['kode_lokasi','periode','kode_gudang','kode_vendor','jenis'],
                        nama : ['Lokasi','Periode','Kode Gudang','Kode Vendor','Jenis'],
                        header : [['Kode', 'Nama'],['Periode', 'Nama'],['Kode', 'Nama'],['Kode', 'Nama'],['Kode']],
                        headerpilih : [['Kode', 'Nama','Action'],['Periode', 'Nama','Action'],['Kode', 'Nama','Action'],['Kode', 'Nama','Action'],['Kode','Action']],
                        columns: [
                            [
                                { data: 'kode_lokasi' },
                                { data: 'nama' }
                        ...
                        url :["{{ url('esaku-report/filter-lokasi') }}","{{ url('esaku-report/filter-periode-keu') }}","{{ url('esaku-report/filter-gudang') }}","{{ url('esaku-report/filter-vendor') }}","{{ url('esaku-report/filter-jenis-saldo') }}"],
                        parameter:[{},{
                        ...
                        display:['kode','name','kode','kode','kode'],
                        pageLength:[10,12,10,10,10]
                    });
    ...
    $('#form-filter').submit(function(e){
        ...
        $formData.append("kode_vendor[]",$kode_vendor.type);
        $formData.append("kode_vendor[]",$kode_vendor.from);
        $formData.append("kode_vendor[]",$kode_vendor.to);
        ...
        xurl = "{{ url('esaku-auth/form/rptSaldoHutang') }}";
    ...
    $("#sai-rpt-excel").click(function(e) {
        ...
        $("#saku-report #canvasPreview").table2excel({
            ...
            name: "Lap_SaldoHutang_{{ Session::get('userLog').'_'.Session::get('lokasi').'_'.date('dmy').'_'.date('Hi') }}",
            filename: "Lap_SaldoHutang_{{ Session::get('userLog').'_'.Session::get('lokasi').'_'.date('dmy').'_'.date('Hi') }}.xls", // do include extension
