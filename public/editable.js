...
function EditableTablePlugin(tableSelector, options) {
    ...
    function render() {
        ...
        dataSlice.forEach((row, i) => {
            ...
            // Row number
            $tr.append($('<td class="text-center">').text(realIndex + 1));

            // Delete button
            if (settings.allowDelete) {
                $tr.append(
                    $('<td class="action-cell text-center">').append(
                        $('<button type="button" class="btn btn-sm delete-row" style="padding:0px !important;"><i class="fa fa-lg fa-trash"></i></button>')
                            .on('click', function (e) {
                                e.stopPropagation();
                                setTimeout(() => {
            ...
            $.each(settings.inputs, function (key, colOpt) {
                ...
                switch (colOpt.type) {
                    ...
                    case 'search' : 
                        ...
                        const $btnSearch = $(`
                            <button class="btn btn-search" type="button" tabindex="-1" style="position:absolute;right:0;padding: 2px 4px !important;top:0px;border: 0px;z-index:3">
                    ...
                    case 'select':
                        ...
                        optionsList.forEach(opt => {
                            ...
                            $input.append($('<option>').val(val).text(label).prop('selected', val == row[key] ?? ""));
                        });
                        displayText = $input.find('option:selected').text();
                        $display.text(displayText).attr(displayText);
                        $input.data('key', key);
    ...
    function applyEvents() {
        ...
        $(document).off("click.editablePlugin").on("click.editablePlugin", function (e) {
            if ($(e.target).closest('.selectize-dropdown, .selectize-input, select.input-select').length || $(e.target).attr('id') == "trigger-bottom-sheet") return;
            $table.find(".edit-input").hide();
            $table.find(".display").show();
            $table.find(".editable-cell").removeClass("editing");
    ...
    function addRow() {
        ...
        const newRow = {};
        Object.keys(settings.inputs).forEach(k => newRow[k] = '');
    ...
    //     var table_id = tableSelector.replace('#','');
    ...
    //             $('<a class="page-link" href="#">').text('«').on('click', function (e) {
    //                 e.preventDefault();
