...
!(function (root, factory) {
    ...
    Typeahead.prototype = {
        ...
        show: function () {
            ...
            return element.css({ top: newTop, left: newLeft }).show(), !0 === this.options.fitToElement && element.css("width", this.$element.outerWidth() + "px"), (this.shown = !0), this;
        },
        hide: function () {
        ...
        highlighter: function (item) {
            ...
            var i,
                ...
                first = [],
                second = [];
            if (matches && matches.length) for (i = 0; i < matches.length; ++i) matches[i].length > 2 && first.push(matches[i]);
            ...
            return (
                $.each(items, function (key, value) {
                    key > 0 && value[_category] !== items[key - 1][_category] && data.push({ __type: "divider" }),
                        !value[_category] || (0 !== key && value[_category] === items[key - 1][_category]) || data.push({ __type: "category", name: value[_category] }),
                        data.push(value);
                }),
                (items = $(data).map(function (i, item) {
                    ...
                    return (
                        ...
                        text == self.$element.val() && (i.addClass("active"), self.$element.data("active", item), (activeFound = !0)),
                        i[0]
                    );
                })),
                this.autoSelect && !activeFound && (items.filter(":not(.dropdown-header)").first().addClass("active"), this.$element.data("active", items.first().data("value"))),
        ...
        listen: function () {
            this.$element
                ...
                this.eventSupported("keydown") && this.$element.on("keydown.bootstrap3Typeahead", $.proxy(this.keydown, this));
